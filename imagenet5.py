...
# although this wont perserve correct shape, i think each filter will keep its fh*fw values and shape
...
l0 = Convolution(input_sizes=[batch_size, 224, 224, 3], filter_sizes=[7, 7, 3, 64], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=0, last_layer=False, name="conv1", load=weights_conv, train=train_conv, fa=args.fa)
l1 = MaxPool(size=[batch_size, 224, 224, 64], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

l2 = Convolution(input_sizes=[batch_size, 112, 112, 64], filter_sizes=[7, 7, 64, 128], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=1., last_layer=False, name="conv2", load=weights_conv, train=train_conv, fa=args.fa)
l3 = MaxPool(size=[batch_size, 112, 112, 128], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

l4 = Convolution(input_sizes=[batch_size, 56, 56, 128], filter_sizes=[7, 7, 128, 128], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=0., last_layer=False, name="conv3", load=weights_conv, train=train_conv, fa=args.fa)
l5 = MaxPool(size=[batch_size, 56, 56, 128], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

l6 = Convolution(input_sizes=[batch_size, 28, 28, 128], filter_sizes=[7, 7, 128, 128], num_classes=num_classes, init_filters=args.init, strides=[1, 2, 2, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=1., last_layer=False, name="conv4", load=weights_conv, train=train_conv, fa=args.fa)
l7 = MaxPool(size=[batch_size, 14, 14, 128], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

l8 = ConvToFullyConnected(shape=[7, 7, 128])

l9 = FullyConnected(size=[7*7*128, 4096], num_classes=num_classes, init_weights=args.init, alpha=learning_rate, activation=act, bias=1., last_layer=False, l2=args.l2, name="fc1", load=weights_fc, train=train_fc, fa=args.fa)
...
for ii in range(0, epochs):
    ...
    for j in range(0, len(train_imgs), batch_size):
        ...
        if (j % (batch_size * 1000) == 0):
            ...
            [(_forward, _backward), _gvs, _weights, _total_correct, _top5, _] = sess.run([backward, grads_and_vars, weights, total_correct, total_top5, train], feed_dict={handle: train_handle, dropout_rate: args.dropout, learning_rate: lr})
            
            img = _forward[0][0, :, :, 0]
            plt.imsave('forward_%d_1_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _forward[2][0, :, :, 0]
            plt.imsave('forward_%d_2_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _forward[4][0, :, :, 0]
            plt.imsave('forward_%d_3_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _forward[6][0, :, :, 0]
            plt.imsave('forward_%d_4_%d.png' % (args.fa, img_num), img, cmap="gray")

            # backwards is in the right order actually because we do backward loop in Model.
            img = _backward[1][0, :, :, 0]
            plt.imsave('backward_%d_1_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _backward[3][0, :, :, 0]
            plt.imsave('backward_%d_2_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _backward[5][0, :, :, 0]
            plt.imsave('backward_%d_3_%d.png' % (args.fa, img_num), img, cmap="gray")

            img = _backward[7][0, :, :, 0]
            plt.imsave('backward_%d_4_%d.png' % (args.fa, img_num), img, cmap="gray")

            # this aint working no more bc we return a gradient for fb.
            # same is not true for gvs
            f5 = _gvs[6][0]
            viz('gv_%d_4_%d.png' % (args.fa, img_num), f5)

            f4 = _gvs[8][0]
            viz('gv_%d_3_%d.png' % (args.fa, img_num), f4)

            f3 = _gvs[10][0]
            viz('gv_%d_2_%d.png' % (args.fa, img_num), f3)

            f2 = _gvs[12][0]
            viz('gv_%d_1_%d.png' % (args.fa, img_num), f2)

            ####
            w4 = _weights['conv4']
            viz('w_%d_4_%d.png' % (args.fa, img_num), f4)

            w3 = _weights['conv3']
            viz('w_%d_3_%d.png' % (args.fa, img_num), f3)

            f2 = _weights['conv2']
            viz('w_%d_2_%d.png' % (args.fa, img_num), f2)

            f1 = _weights['conv1']
            viz('w_%d_1_%d.png' % (args.fa, img_num), f1)
