...
batch_per_epoch = int(TRAIN_EXAMPLES / BATCH_SIZE)

for ii in range(EPOCHS):
    epochs.append(ii)
    
    for jj in range(batch_per_epoch):
        batches.append(ii * batch_per_epoch + jj)
        ...
        if args.angles:
            ...
            angle = angle_between(B, W2) * (180. / 3.14)
            angles.append(angle)
    ...
    acc = total_correct_examples / total_examples
    accs.append(acc)    
    
    if args.angles:
        print ("acc : %f, angle : %f" % (acc, angle))
    else:
        print ("acc : %f" % (acc))
