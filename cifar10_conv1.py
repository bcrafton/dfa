...
parser.add_argument('--name', type=str, default="cifar10_1")
...
l4 = FullyConnected(size=[16*16*96, 10], num_classes=10, init_weights=args.init, alpha=learning_rate, activation=Linear(), bias=bias, last_layer=True, name='fc3', load=weights_fc, train=train_fc)
...
filename = args.name + str(args.alpha) + '.results'
f = open(filename, "w")
f.write(filename + "\n")
...
if args.save:
    ...
    np.save(args.name + str(args.alpha), w)
