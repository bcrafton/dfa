...
small_w = tf.gather_nd(abs_w, small_i)

large_i = tf.gather(vld_i, sorted_i, axis=0)
large_i = tf.cast(large_i, tf.int32)
large_i = tf.slice(large_i, [10, 0], [15, 2])
large_w = tf.gather_nd(abs_w, large_i)

# new_i = vld_i
new_i = tf.where(abs_w <= 0)
new_i = tf.random_shuffle(new_i)
new_i = tf.slice(new_i, [0, 0], [10, 2])

################################################

[_abs_w, _vld_i, _vld_w, _sorted_i, _small_i, _small_w, _large_w, _new_i] = sess.run([abs_w, vld_i, vld_w, sorted_i, small_i, small_w, large_w, new_i], feed_dict={weights: x})
