...
slices = tf.transpose(slices, [0, 2, 3, 1, 4, 5])
slices = tf.reshape(slices, [1, 9, 3, 3, 9, 3])
slices = tf.transpose(slices, [0, 3, 1, 2, 4, 5])

slices = tf.transpose(slices, [0, 1, 3, 2, 4, 5])
slices = tf.reshape(slices, [1, 3, 3, 3, 27, 3])
slices = tf.transpose(slices, [0, 1, 3, 2, 4, 5])
