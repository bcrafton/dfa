...
F = tf.placeholder(tf.float32, [1, 1024, 27, 64])

img = tf.extract_image_patches(images=X, ksizes=[1, 3, 3, 1], strides=[1, 1, 1, 1], rates=[1, 1, 1, 1], padding='SAME')
shape = tf.shape(img)
...
f = np.random.uniform(size=(1, 1024, 27, 64))
[_img, _Y, _O] = sess.run([img, Y, O], feed_dict={X: x, F: f})
print (np.shape(_img), np.shape(_Y), np.shape(_O))
