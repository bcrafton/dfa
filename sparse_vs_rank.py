...
def run_command(benchmark, sparse, rank, name):
    ...
    # cmd = "python mnist_fc.py --epochs 100 --batch_size 64 --alpha 0.05 --gpu %d --dfa 1 --sparse %d --rank %d --init zero --opt gd --save 1 --name %s" % (gpu, sparse, rank, name)
    cmd = "python cifar10_fc.py --epochs 200 --batch_size 64 --alpha 0.005 --gpu %d --dfa 1 --sparse %d --rank %d --init zero --opt adam --save 1 --name %s" % (gpu, sparse, rank, name)
