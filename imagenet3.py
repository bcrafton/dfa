...
l2 = Convolution(input_sizes=[batch_size, 27, 27, 96], filter_sizes=[11, 11, 96, 256], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=1., last_layer=False, name="conv2", load=weights_conv, train=train_conv, fa=args.fa)
l3 = MaxPool(size=[batch_size, 27, 27, 256], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="VALID")

l4 = Convolution(input_sizes=[batch_size, 13, 13, 256], filter_sizes=[7, 7, 256, 256], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=0., last_layer=False, name="conv3", load=weights_conv, train=train_conv, fa=args.fa)

l5 = Convolution(input_sizes=[batch_size, 13, 13, 256], filter_sizes=[7, 7, 256, 256], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=1., last_layer=False, name="conv4", load=weights_conv, train=train_conv, fa=args.fa)

l6 = Convolution(input_sizes=[batch_size, 13, 13, 256], filter_sizes=[7, 7, 256, 256], num_classes=num_classes, init_filters=args.init, strides=[1, 1, 1, 1], padding="SAME", alpha=learning_rate, activation=Relu(), bias=1., last_layer=False, name="conv5", load=weights_conv, train=train_conv, fa=args.fa)
...
for ii in range(0, epochs):
    ...
    for j in range(0, len(train_imgs), batch_size):
        ...
        if (j % (batch_size * 1000) == 0):
            ...
            img = _forward[6][0, :, :, 0]
            plt.imsave('forward_%d_5_%d.png' % (args.fa, img_num), img, cmap="gray")
            ...
            img = _backward[7][0, :, :, 0]
            plt.imsave('backward_%d_5_%d.png' % (args.fa, img_num), img, cmap="gray")

            # this aint working no more bc we return a gradient for fb.
            # same is not true for gvs
            f5 = _gvs[6][0]
            viz('gv_%d_5_%d.png' % (args.fa, img_num), f5)

            f4 = _gvs[8][0]
            viz('gv_%d_4_%d.png' % (args.fa, img_num), f4)

            f3 = _gvs[10][0]
            viz('gv_%d_3_%d.png' % (args.fa, img_num), f3)

            f2 = _gvs[12][0]
            viz('gv_%d_2_%d.png' % (args.fa, img_num), f2)

            f1 = _gvs[14][0]
            viz('gv_%d_1_%d.png' % (args.fa, img_num), f1)
